...
class GoTrueClient {
  ...
  Future<AuthResponse> signUp({
    ...
    if (email != null) {
      ...
      response =
          await _fetch.request('$_url/signup', RequestMethodType.post, options: fetchOptions) as Map<String, dynamic>;
    } else {
  ...
  Future<AuthSessionUrlResponse> exchangeCodeForSession(String authCode) async {
    ...
    final authSessionUrlResponse =
        AuthSessionUrlResponse(session: Session.fromJson(response)!, redirectType: redirectType?.name);
  ...
  Future<void> _autoRefreshTokenTick() async {
    try {
      ...
      final expiresInTicks = (DateTime.fromMillisecondsSinceEpoch(expiresAt * 1000).difference(now).inMilliseconds /
  ...
  Future<AuthResponse> _refreshAccessToken(String refreshToken) async {
    ...
    return await retry<AuthResponse>(
      ...
      () async {
        attempt++;
        final options = GotrueRequestOptions(
            headers: _headers, body: {'refresh_token': refreshToken}, query: {'grant_type': 'refresh_token'});
      ...
      retryIf: (e) {
        ...
        return e is AuthRetryableFetchException &&
            (DateTime.now().millisecondsSinceEpoch + nextBackOff.inMilliseconds - startedAt.millisecondsSinceEpoch) <
                Constants.autoRefreshTickDuration.inMilliseconds;
      },
